...
* Philippe Bocquier
* To deal with: 
* 1. missing data at some t's
* 2. multiple events considering return to detectable viremia

use COVID19_HCQ_v7, clear

rename _t orig_t

keep vfree* id2 age arm sex clinicalstatus azithro_treat d_censor note arm2 hcq_s1 hcq_s2 orig_t

reshape long vfree, i(id2) j(day)
drop if day==0
* correcting coding error (label do not correspond to codes) of vfree variable
gen vfree_rec=vfree-1
replace vfree_rec=. if vfree_rec==2

stset day, id(id2) fail(vfree_rec==1) // to reproduce the same as with long-format data

stcox ib3.arm2 age i.sex // ib2.clinical

* Note: no HCQ given to under age<20
stcox ib3.arm2 age i.sex if age>=20 // ib2.clinical

* Accounting for missing days of observation and repeatable events
capture drop cumvfree_rec
bysort id2 (day): gen cumvfree_rec=sum(vfree_rec==1) if vfree_rec==1
bysort id2 (day): replace vfree_rec=. if cumvfree_rec==cumvfree_rec[_n-1]+1 & vfree_rec==1
drop cumvfree_rec
bysort id2 (day): gen spell=sum(vfree_rec[_n-1]==1) 
sort id2 day

stset day if vfree_rec!=., id(id2) fail(vfree_rec==1) exit(time .)

capture drop sch* 
capture drop sca*
stcox ib3.arm2 age i.sex ib0.spell /// ib2.clinical
		, schoenfeld(sch*) scaledsch(sca*) 
estat phtest, detail

stphplot, by(arm2) adjust(age sex spell clinical) nonegative title(Nelson-Aalen cumulative hazard estimates) ///
		sav(stphplot, replace) plot1opts(connect(stairstep) msymbol(i)) ///
		plot2opts(connect(stairstep) msymbol(i)) plot3opts(connect(stairstep) msymbol(i)) 
		
stcurve, surv at1(arm2=1) at2(arm2=2) at3(arm2=3) lwidth(1.5 1.5 1.5) ///
		legend(label (1 "HCQ") label (2 "HCQ-AZ") label (3 "Control")) ///
		legend(order(1 2 3)) scheme (plottig) /// 
		xtitle("Days", margin(vsmall) size(5)) ///
		ytitle("Proportion with undetectable" "viremia via PCR", size(5)) ///
		xlabel(, angle(h) labsize(4)) ///
...
recode age (min/19=1 "10-19") (20/39=2 "20-39") (40/59=3 "40-59") (60/max=4 "60+"), gen(agegroup)

* Note: no HCQ given to under age<20
capture drop sch* 
capture drop sca*
stcox ib3.arm2 age i.sex ib0.spell if age>=20 /// ib2.clinical
		, schoenfeld(sch*) scaledsch(sca*) 
estat phtest, detail
*suspected non proportionality in sex and spell
stphplot, by(arm2) adjust(age sex spell) nonegative title(Nelson-Aalen cumulative hazard estimates) ///
		sav(stphplot, replace) plot1opts(connect(stairstep) msymbol(i)) ///
		plot2opts(connect(stairstep) msymbol(i)) plot3opts(connect(stairstep) msymbol(i)) 
stphplot, by(spell) adjust(age sex arm2) nonegative title(Nelson-Aalen cumulative hazard estimates) ///
		sav(stphplot, replace) plot1opts(connect(stairstep) msymbol(i)) ///
		plot2opts(connect(stairstep) msymbol(i)) 
stphplot, by(sex) adjust(age spell arm2) nonegative title(Nelson-Aalen cumulative hazard estimates) ///
		sav(stphplot, replace) plot1opts(connect(stairstep) msymbol(i)) ///
		plot2opts(connect(stairstep) msymbol(i)) 
		
capture drop sch* 
capture drop sca*
stcox ib3.arm2 ib2.agegroup i.sex ib0.spell if age>=20 /// ib2.clinical
		, schoenfeld(sch*) scaledsch(sca*) 
estat phtest, detail
*suspected non proportionality in spell
stphplot, by(arm2) adjust(age sex spell) nonegative title(Nelson-Aalen cumulative hazard estimates) ///
		sav(stphplot, replace) plot1opts(connect(stairstep) msymbol(i)) ///
		plot2opts(connect(stairstep) msymbol(i))  plot3opts(connect(stairstep) msymbol(i))
stphplot, by(spell) adjust(age sex arm2) nonegative title(Nelson-Aalen cumulative hazard estimates) ///
...
stcurve, surv at1(arm2=1) at2(arm2=2) at3(arm2=3) lwidth(1.5 1.5 1.5) ///
		legend(label (1 "HCQ") label (2 "HCQ-AZ") label (3 "Control")) /// PhB : correction for codes
		legend(order(1 2 3)) scheme (plottig) /// 
		xtitle("Days", margin(vsmall) size(5)) ///
		ytitle("Proportion with undetectable" "viremia via PCR", size(5)) ///
		xlabel(, angle(h) labsize(4)) ///
...
* results far from robust
stcox ib1.arm if age>=20, vce(bootstrap, reps(500)) // ib2.clinical
stcox ib3.arm2 if age>=20, vce(bootstrap, reps(500)) // ib2.clinical
stcox ib3.arm2 age i.sex ib0.spell if age>=20, vce(bootstrap, reps(500)) // ib2.clinical
stcox ib3.arm2 ib2.agegroup i.sex ib0.spell if age>=20, vce(bootstrap, reps(500)) // ib2.clinical
